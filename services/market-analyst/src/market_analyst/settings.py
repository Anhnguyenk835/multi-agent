...
class MarketAnalystAISettings:
    ...
    @classmethod
    def from_environment(cls) -> "MarketAnalystAISettings":
...
@dataclass(frozen=True, slots=True)
class MarketAnalystExaSettings:
    ...
    @classmethod
    def from_environment(cls) -> "MarketAnalystExaSettings":
        return cls(
...
@dataclass(frozen=True, slots=True)
class MarketResearchSettings:
    backend_base_url: str = "http://backend:8010"
    backend_timeout_seconds: float = 15.0
    source_excerpt_characters: int = 4_000
    backend_api_token: str | None = None

    @classmethod
    def from_environment(cls) -> "MarketResearchSettings":
        return cls(
            backend_base_url=os.getenv("BACKEND_BASE_URL", "http://backend:8010").rstrip("/"),
            backend_timeout_seconds=max(1.0, float(os.getenv("BACKEND_TIMEOUT_SECONDS", "15"))),
            source_excerpt_characters=max(
                500, int(os.getenv("MARKET_SOURCE_EXCERPT_CHARACTERS", "4000"))
            ),
            backend_api_token=os.getenv("BACKEND_INTERNAL_API_TOKEN") or None,
        )


@dataclass(frozen=True, slots=True)
class DemoSettings:
    ai: MarketAnalystAISettings = field(default_factory=MarketAnalystAISettings)
    exa: MarketAnalystExaSettings = field(default_factory=MarketAnalystExaSettings)
    market: MarketResearchSettings = field(default_factory=MarketResearchSettings)

    @classmethod
    def from_environment(cls) -> "DemoSettings":
        return cls(
            ai=MarketAnalystAISettings.from_environment(),
            exa=MarketAnalystExaSettings.from_environment(),
            market=MarketResearchSettings.from_environment(),
        )
