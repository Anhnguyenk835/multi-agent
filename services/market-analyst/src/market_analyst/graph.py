...
from market_analyst.errors import (
    GroundingError,
    InvalidOutputError,
    ToolCallLimitReachedError,
)
from market_analyst.llm import build_chat_model as _build_chat_model
from market_analyst.llm import model_timeouts as _model_timeouts
from market_analyst.llm_schema import LLMFindingsResponse
from market_analyst.prompts import SYSTEM_PROMPT
from market_analyst.search import parse_tool_message
from market_analyst.settings import DemoSettings, MarketAnalystAISettings
from market_analyst.telemetry import ReactAgentTelemetry, extracted_request_context, operation_span
from market_analyst.tools import build_search_tool


def _deadline_middleware(settings: MarketAnalystAISettings, deadline_at: datetime | None):
    @wrap_model_call
    async def apply_deadline(request, handler):
        ...
        if gateway_timeout <= 0 or client_timeout <= 0:
            raise TimeoutError("market analyst deadline reached before model call")
...
_MARKET_ANALYST_INPUT_FIELDS = set(MarketAnalystInput.model_fields)


def _extract_request(state: dict[str, object]) -> MarketAnalystInput:
    return MarketAnalystInput.model_validate(
        {key: value for key, value in state.items() if key in _MARKET_ANALYST_INPUT_FIELDS}
    )
...
def _build_graph(runtime_settings: DemoSettings):
    ...
    async def run_react_agent(
        state: MarketAnalystRemoteState,
        ...
        with (
            ...
            operation_span(
                "market-analyst.run_react_agent",
                observation_type="agent",
                attributes={"app.agent": "market_analyst", "app.attempt": request.attempt},
            ),
        ):
...
def _deadline_response(request: MarketAnalystInput) -> dict[str, object]:
    return MarketAnalystOutput(
        **copy_request_metadata(request),
        status=ContractStatus.FAILED,
        error=ContractError(
            code=ErrorCode.DEADLINE_EXCEEDED,
            message="Market Analyst deadline exceeded",
            retryable=True,
        ),
