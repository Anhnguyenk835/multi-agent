...
import pytest
from distributed_agent_contracts import ContractStatus, MarketAnalystInput, MarketAnalystOutput
from langchain_core.language_models.chat_models import BaseChatModel
from langchain_core.messages import AIMessage, BaseMessage, ToolCall
from langchain_core.outputs import ChatGeneration, ChatResult
from market_analyst.errors import GroundingError, InvalidOutputError, ToolCallLimitReachedError
from market_analyst.graph import (
    _model_timeouts,
    _raise_missing_structured_output,
    build_graph,
    build_graph_for_settings,
)
from market_analyst.llm_schema import LLMFindingsResponse
from market_analyst.settings import (
    DemoSettings,
    MarketAnalystAISettings,
    MarketAnalystExaSettings,
)
...
def _live_demo_settings() -> DemoSettings:
    return DemoSettings(
        ai=MarketAnalystAISettings(gateway_api_key="test-key"),
        exa=MarketAnalystExaSettings(exa_api_key="test-exa-key"),
    )
...
@pytest.mark.anyio
async def test_market_analyst_grounds_findings_in_tool_results(monkeypatch) -> None:
    import market_analyst.search as search_module
    ...
    monkeypatch.setattr("market_analyst.graph._build_chat_model", lambda settings: fake_model)

    request = MarketAnalystInput(
        request_id=uuid4(), trace_id="live-trace", query="AI coding agents"
    )
    result = await build_graph_for_settings(_live_demo_settings()).ainvoke(
        request.model_dump(mode="json")
    )
    response = MarketAnalystOutput.model_validate(result)
...
@pytest.mark.anyio
async def test_market_analyst_rejects_unknown_source_tag(monkeypatch) -> None:
    import market_analyst.search as search_module
    ...
    request = MarketAnalystInput(
        request_id=uuid4(), trace_id="live-trace-2", query="AI coding agents"
    )
...
@pytest.mark.anyio
async def test_market_analyst_raises_when_search_tool_call_limit_reached(monkeypatch) -> None:
    ...
    request = MarketAnalystInput(
        request_id=uuid4(), trace_id="limit-trace", query="AI coding agents"
    )
...
@pytest.mark.anyio
async def test_market_analyst_graph_factory_accepts_langgraph_server_config(monkeypatch) -> None:
    ...
    assert MarketAnalystOutput.model_validate(result).status is ContractStatus.SUCCESS
