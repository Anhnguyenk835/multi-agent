...
from market_analyst.search import ExaSourceResult, parse_tool_message
from market_analyst.settings import MarketAnalystExaSettings
...
async def test_search_tool_returns_tagged_results_and_strips_www() -> None:
    settings = MarketAnalystExaSettings(exa_api_key="test-key", exa_max_results=2)
...
async def test_search_tool_preserves_long_provider_tool_call_id() -> None:
    ...
    tool = build_search_tool(
        MarketAnalystExaSettings(exa_api_key="test-key"),
...
async def test_search_tool_has_logical_and_external_search_spans(monkeypatch) -> None:
    ...
    with telemetry.operation_span("market-analyst.run_react_agent"):
        await tool.ainvoke(
            {
                "args": {"query": "AI coding agents"},
                "type": "tool_call",
