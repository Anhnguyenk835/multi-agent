...
# prefixes its own (writer-*) to avoid colliding with the other services.
# The virtual key is provisioned by infra/llm-gateway/scripts.
for KEY in LLM_GATEWAY_API_KEY; do
  grep "^$KEY=" ../../../.env | cut -d= -f2- | tr -d '\n' \
    | gcloud secrets versions add "writer-$KEY" --data-file=-
done
