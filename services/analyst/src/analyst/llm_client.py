...
def _default_openai_client(settings: AnalystAISettings) -> openai.AsyncOpenAI:
    # wrap_openai adds a traced "ChatOpenAI" child run with token usage.
    return wrap_openai(openai.AsyncOpenAI(api_key=settings.openai_api_key))
...
def _safe_usage(response: object) -> dict[str, int]:
...
async def generate_structured[T: BaseModel](
    *,
    ...
    with trace_operation(
        "analyst.llm",
        metadata={"service": "analyst", "provider": "openai", "model": model, "schema": schema.__name__},
        run_type="llm",
        inputs={
            "provider": "openai",
            "model": model,
