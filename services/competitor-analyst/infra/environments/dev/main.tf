...
resource "google_service_account" "runtime" {
  project      = var.project_id
  account_id   = "competitor-analyst-runtime"
  display_name = "Competitor Analyst Cloud Run runtime identity"
}
...
module "artifact_registry" {
  ...
  region        = var.region
  repository_id = "competitor-analyst"
}

module "secrets" {
  source                         = "../../modules/secrets"
  project_id                     = var.project_id
  name_prefix                    = "competitor-analyst-"
...
module "cloud_run" {
  ...
  project_id            = var.project_id
  region                = var.region
  service_name          = "competitor-analyst"
  image                 = var.image
  ...
  env_vars = {
    ...
    LLM_MODEL_ROUTE            = "market-standard"
