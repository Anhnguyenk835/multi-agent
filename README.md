...
services/
  ...
  market-analyst/      LangGraph RemoteGraph-compatible market analyst
  competitor-analyst/  Google ADK gRPC competitor analyst
  analyst/             LangChain HTTPS analysis service
  writer/              HTTPS executive-brief service
packages/
  contracts/           Shared Pydantic contracts and generated gRPC artifacts
...
its own runtime dependencies and owns the boundary schemas it produces or
consumes. `packages/contracts` contains shared wire models, but does not force
gRPC/Protobuf onto HTTP-only services; those dependencies belong to the
gRPC-producing and consuming services.
...
| --- | --- | --- |
| Market Analyst | LangGraph RemoteGraph at `http://localhost:8001`, graph `market_analyst` | `GET /ok` |
| Competitor Analyst | gRPC `distributed_agents.competitor.v1.CompetitorAnalyst` on `localhost:50051` | gRPC Health Checking |
...
| Backend | Market dashboard reads on `http://localhost:8010/api` | `GET /ready` |
| Dashboard PostgreSQL | Saved dashboard data on `localhost:54321` | `pg_isready` |
...
Market Analyst search/source events, Writer prose deltas, and a terminal workflow
...
Vite proxies `/api` to the local Orchestrator on port 8000 and
`/backend-api` to the Backend on port 8010. Start the dashboard data
services with:
...
For a separately hosted frontend, set `ORCHESTRATOR_ALLOWED_ORIGINS` and
`BACKEND_ALLOWED_ORIGINS` to their comma-separated origin lists. Agent
activities are product-defined summaries; prompts,
...
Valid service names are `MARKET_ANALYST`, `COMPETITOR_ANALYST`, `ANALYST`, and `WRITER`.
