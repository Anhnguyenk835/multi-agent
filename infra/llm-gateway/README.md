...
Copy `.env.example` to `.env`, set the four gateway-only secrets, then start
the stack from the repository root:

```bash
docker compose up --build llm-gateway llm-gateway-postgres
```
...
terraform init
terraform apply -target=module.artifact_registry -target=module.cloud_sql -target=module.secrets
```

Seed `llm-gateway-OPENAI_API_KEY`, `llm-gateway-GEMINI_API_KEY`,
`llm-gateway-ANTHROPIC_API_KEY`, and `llm-gateway-LITELLM_MASTER_KEY` in Secret
Manager, then run `terraform apply`. Set each agent environment's
